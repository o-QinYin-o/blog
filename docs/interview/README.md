...
key帮助React识别哪些元素改变了，比如被添加和删除。给每个元素赋予一个确定的标识，根据key来决定销毁重新创建组件还是跟新组件，提高性能。

### redux定义

Redux 是一种状态管理工具。尽管它主要与 React 搭配使用，但也可以与其他任何 JavaScript 框架或库搭配。对存储的更新将触发与存储的已更新部分连接的组件的重新渲染。
...
事先定义一个用于获取跨域相应数据的回调函数，并通过没有同源策略限制的`script`标签发起一个请求（将毁掉函数的名称放到这个请求的`query`参数里），然后服务端返回这个回调函数的执行，并将需要响应的数据放到回调函数的参数里，前端的`script`标签请求到这个执行的回调函数后会立马执行，于是就拿到了执行的响应数据。
...
1. 第一次握手：建立连接时，客户端发送sycn包到服务器，并进入SYN_SEN（同步序列编号）状态，等待服务器缺人；
2. 第二次握手，服务器受到sync包并确认客户的SYN，同时也发送一个自己的SYN包，即SYN+ACK包，此时服务器进入SYN_RECV状态；
3. 第三次握手，客户端收到服务器的SYN+ACK包，向服务器发送确认包ACK，此包发送完毕，客户端和服务器进入ESTABLISHED（TCP连接状态成功）状态，完成三次握手
